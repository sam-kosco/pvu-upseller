...
# ─────────────────────────────────────────────────────────────
# Display name overrides — internal name stays "Polymer Coating"
# (JotForm + tracker unchanged) but PDF/DOCX shows "Nu-Glaze"
# ─────────────────────────────────────────────────────────────
DISPLAY_NAMES = {
...
# Rendered right after the supersection boilerplate text.
# ─────────────────────────────────────────────────────────────
...
INCLUDE_EXAMPLE_PHOTOS = {
    "Brightwork":        False,   # now at supersection level
    "Ceramic Coating":   False,   # now at supersection level
    "Permagard Coating": False,   # now at supersection level
    "Polymer Coating":   False,   # now at supersection level
    "Interior Detail":   False,
    "Exterior Detail":   False,
    "Carpet Extraction": False,   # now at supersection level
    "Xylon":             False,
}
...
def rewrite_notes(raw_notes: str, service_name: str, condition_photos: list) -> str:
    ...
    content = []
    instruction = (
        f"You are writing a single paragraph for a professional aircraft detailing proposal "
...
if __name__ == "__main__":
    ...
    fake_data = {
        ...
        "upsells": [
            ...
            {"service": "Polymer Coating", "price": "6400",  "notes": "existing coating worn",
             "photos": []},
    ...
    out_path = os.path.join(tempfile.gettempdir(), "upsell_test.pdf")
    out = generate_pdf(fake_data, graph_token="FAKE", output_path=out_path)
    print(f"Done -> {out}")
